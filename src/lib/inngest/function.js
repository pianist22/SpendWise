...
export const checkBudgetAlert = inngest.createFunction(
    ...
    {cron:"0 */6 * * *"},// this will fire the cron job after every 6 hours
  async ({step }) => {
    // Here we are fetching the default accounts for all of the user
    ...
    // here we will check if the expenses exceeds the budget amount
    for(const budget of budgets){
        ...
        await step.run(`check-budget-${budget.id}`,async()=>{
            ...
            if(percentageUsed>=80 && 
                ...
                // update the budget last alert sent date
